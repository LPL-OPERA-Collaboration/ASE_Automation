...
% --- Title ---
\title{Automated ASE Measurement User Manual}
\date{\today}
...
\begin{enumerate}
    ...
    \item \textbf{Horiba SDRIVE-500} (Shutter Switch)
    \item \textbf{QC Sapphire} (Pulser Switch)
    \item \textbf{N2 Laser} (Turn Key)
    \item \textbf{Thorlabs Motor} (Powered via PC)
\end{enumerate}

\subsection{2. Manual Alignment}
Before running the codes, align the system manually.

\begin{enumerate}
    \item Open \textbf{LabSpec 6}, \textbf{ELLO} (Motor), and \textbf{QC\_9200} (Pulser, Optional).
...
\subsection{3. Directory \& File Structure}
After alignment, navigate to the project folder to locate the scripts.

\textbf{Base Directory:}
\begin{lstlisting}
C:\Users\Equipe_OPAL\Documents\GitHub\LPL-OPERA-Collaboration\ASE_Automation
\end{lstlisting}

\textbf{Working Folder (LPL):}
All operations for this experiment are performed within the \textbf{LPL} folder.

\dirtree{%
.1 ASE\_Automation.
.2 LPL \textit{(Current Working Directory)}.
.3 aquisition codes.
.4 aquisition\_config.py \textbf{(Edit this)}.
.4 main\_measurement.py \textbf{(Run this)}.
.4 elliptec\_motor\_controller.py.
.4 horiba\_spectrometer\_controller.py.
.4 sapphire\_pulser\_controller.py.
.3 analysis codes.
.4 analysis\_config.py \textbf{(Edit this)}.
.4 step1\_energy\_calc.py.
.4 step2\_signal\_processing.py.
.4 step3\_spectrum\_analysis.py.
.3 callibration.py \textbf{(For Filter Wheel)}.
.3 file\_location\_memo.py.
.2 OPERA.
.2 shared.
}
